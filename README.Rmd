...
Currently `podaac-data-downloader` only [downloads the global file](https://github.com/podaac/data-subscriber/issues/134#issuecomment-1546155900), but there are plans afoot to add a `--subset` argument so only portions of the global raster are tranferred.  Until then we download the entire globe, but then provide tools for subsetting.
...
Without intentional cleanup, the download directory may grow to an unreasonable size. We provide a function to purge that directory as needed.
